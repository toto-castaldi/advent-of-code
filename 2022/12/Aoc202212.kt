...
class Aoc202212 () {
    ...
    fun shortestPath(): Int {
        ...
        BidimentionalNode.navigate(topLeft) {
            ...
            from.u() ?.let { dest -> if (valid(from.data, dest.data)) dijkstra.arc(from.data, dest.data, 1)}
            from.d() ?.let { dest -> if (valid(from.data, dest.data)) dijkstra.arc(from.data, dest.data, 1)}
            from.l() ?.let { dest -> if (valid(from.data, dest.data)) dijkstra.arc(from.data, dest.data, 1)}
            from.r() ?.let { dest -> if (valid(from.data, dest.data)) dijkstra.arc(from.data, dest.data, 1)}
        }
